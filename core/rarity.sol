...
// AQUI EMPIEZA EL CONTRATO RARITY

contract rarity is ERC721Enumerable {
    // numero del siguiente personaje (se usa para mintear el NFT-721)
    ...
    // registro de experiencia de cada personaje
    mapping(uint256 => uint256) public xp;

    // registro de logs de cada personaje
    mapping(uint256 => uint256) public adventurers_log;

    // registro enlaza cada personaje con su clase
    mapping(uint256 => uint256) public class;

    // registro enlaza cada personaje con su nivel
    mapping(uint256 => uint256) public level;

    // Eventos que se emiten cuando se crea un personaje y cuando suben de nivel
    ...
    //  funcion que sube de experiencia al personaje.
    function adventure(uint256 _summoner) external {
        // Comprueba que el que llama a la funcion sea el dueño del personaje
        require(_isApprovedOrOwner(msg.sender, _summoner));
        // Comprueba que ha pasado mas de un dia desde el ultimo log del personaje
        require(block.timestamp > adventurers_log[_summoner]);
        // se actualiza el log
        adventurers_log[_summoner] = block.timestamp + DAY;
        // se le suma la xp al personaje
        xp[_summoner] += xp_per_day;
    }

    // funcion que gasta la experiencia al personaje
    function spend_xp(uint256 _summoner, uint256 _xp) external {
        // Comprueba que el que llama a la funcion sea el dueño del personaje
        require(_isApprovedOrOwner(msg.sender, _summoner));
        // se le resta la xp al personaje
        xp[_summoner] -= _xp;
    }
    ...
    function level_up(uint256 _summoner) external {
        ...
        uint256 _level = level[_summoner]; // nivel actual del personaje
        uint256 _xp_required = xp_required(_level); // experiencia requerida para siguiente nivel
        xp[_summoner] -= _xp_required;
        level[_summoner] = _level + 1;
        emit leveled(msg.sender, level[_summoner], _summoner);
    }

    // funcion que devuelve toda la informacion de un personaje (experiencia, nivel, clase y ultima sesion)
    ...
    // Aquí se está creando el personaje
    ...
    // funcion que calcula matematicamente la experiencia hasta el siguiente nivel
    ...
    // funcion que devuelve los metadatos del personaje (imagen negra con la clase, nivel y la experiencia en formato texto)
    // los metadatos estan en la blockchain, ya que se generan al vuelo codificados en base64
    ...
    // pasa un digito formato numero a formato string
    function toString(uint256 value) internal pure returns (string memory) {
